...
2. **Server**: Retrieves the cached response for the relevant API query and returns the rendered HTML containing the cached data.
3. **Server**: After sending the response, checks if the cached data is stale. If it is, it fetches fresh data from the API to update the cache for future requests.
4. **Browser**: Hydrates the page with the cached data and simultaneously initiates a client-side request to fetch the latest data.
