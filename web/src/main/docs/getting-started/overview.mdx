...
In Client Components, `clientQuery` can be used to fetch data in the following ways:

<Graphics.QueryPatterns.Render
  queryOptions={["client_fresh", "client_cached", "client_swr"]}
  size="full"
/>
...
In Server Components, `serverQuery` can be used to fetch data in the following ways:

<Graphics.QueryPatterns.Render
  queryOptions={["server_fresh", "server_cached"]}
  size="full"
/>
