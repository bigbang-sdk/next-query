...
export function TabsCodeBlock({ defaultValue, tsLogo = true, children }: T_TABS_CODE_BLOCK) {
  ...
  return (
    <Tabs
      ...
      className="w-full bg-primary-foreground rounded-0 pb-3"
    >
      <TabsList className="w-full border-b border-muted px-4 bg-transparent flex items-center justify-between">
        <div className="flex items-center justify-start gap-x-3">
          {tsLogo && <SiTypescript className="w-3.5 h-3.5 text-text mt-0.5" />}
          <div className="flex items-center gap-x-5">
            {items.map((item) => (
              <TabsTrigger
                key={item.props.value}
                value={item.props.value}
                className="w-fit mt-2 pb-2 flex items-center gap-x-2"
              >
                {item.props.label}
              </TabsTrigger>
            ))}
      ...
      {items.map((item) => (
        <TabsContent
          key={item.props.value}
          value={item.props.value}
          ref={item.props.value === selectedTab ? tabContentRef : null}
          className="px-4 pt-2 pb-1 tabs-block"
        >
          {item.props.children}
        </TabsContent>
      ))}
