...
const { CANVAS_WIDTH, BODY_HEIGHT, HEADER_HEIGHT, BOX_HEIGHT, CANVAS_SIZE, DEFAULT_CANVAS_SIZE } = CANVAS_DEFAULTS;
...
const CIRCLE_ICON_SIZE = 4 * 5 * SCALE;
...
const xCirclePartitionTop = (boxIndex: number): number => xPartitionTop(boxIndex) - CIRCLE_ICON_SIZE / 2;
const xCirclePartitionMidPoint = (boxIndex: number): number => xPartitionMidPoint(boxIndex) - CIRCLE_ICON_SIZE / 2;
...
const yCirclePartitionFraction = (patternOption: T_PATTERN_OPTION, index: number, fraction: number): number => yPartitionFraction(patternOption, index, fraction) - CIRCLE_ICON_SIZE / 2;
